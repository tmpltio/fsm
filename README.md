...
This is tag type defined as follows:
```c++
template<typename ... T>
struct type_tag_t
{
...
It is used to deduce types internally in state machine and in [state visitor](#state_visitor), e.g.:
```c++
class visitor
{
public:
    template<typename State>
    auto operator()(type_tag_t<State>)
    {
        /* visitor body */
    }
};
...
* is nothrow move constructible[^2],
...
* is regular invocable[^4] with all possible events argument (```transition::event_t const&```) for all transitions from this state,
* its ```operator()``` returns ```std::optional``` with ```std::variant``` holding all possible destination states for ```state``` from which it was constructed as template parameter for ```type_tag_t``` (```std::optional<std::variant<type_tag_t<transition::destination_t>...>>```).

#### ```state_machine```

Specifies requirements for object that is state machine implementation:
* is nothrow move constructible[^2],
* has public member type ```invalid_state_t``` which is type of state into which transition occurs when current event is not handled by current state or destination state from current transition (```transition::destination_t```) was not added to state machine,
* has public member function ```process_event``` callable with all possible events argument (```transition::event_t const&```) from all transitions from all states that were used to create this state machine implementation and returns value convertible to[^5] ```bool``` for all those calls.

#### ```state_visitor```

Specifies requirements for object that is state visitor:
* is invocable[^4] with all possible states (packed into ```tag_type_t```) - with addition of ```state_machine::invalid_state_t``` - argument (```tag_type_t</* State */>```) that were used to create state machine implementation.
...
Additional public static template member function ```create_default_transition``` is available with only one user provided template parameter ```Destination```. It uses ```transition_factory::default_event_t``` as event for created transition. Such transition will be considered as candidate if there will be no transition for given event in current state.

This factory has also public friend implementations of ```operator|``` which can be used to add guard and action to created transition (each transition can have one guard and one action), e.g.:
```c++
...
Initial state of state machine is first ```state``` object given as argument to ```create_state_machine``` method.
...
Now we can create transitions (for both states transitions are the same, so we can reuse them):
```c++
...
We can now create ```state_visitor```:
```c++
class visitor
{
public:
...
```c++
auto v = visitor{};
machine.visit_state(v);
```

Live example is available on [godbolt.org](https://godbolt.org/z/rK3x9PfMr).
...
```c++
constexpr transition auto to_unmatched = transition_factory::create_default_transition<unmatched>();
...
constexpr transition auto to_self_matched = transition_factory::create_default_transition<matched>();
```
...
Live example is available on [godbolt.org](https://godbolt.org/z/nqnTbMr1G).
...
```c++
while(sm.process_event(tmplt::fsm::transition_factory::default_event_t{}));
```

Live example is available on [godbolt.org](https://godbolt.org/z/c1ha5hs45).

[^1]: [https://en.wikipedia.org/wiki/Finite-state_machine]
[^2]: [https://en.cppreference.com/w/cpp/concepts/move_constructible]
